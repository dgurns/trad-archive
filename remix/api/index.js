...
var loader8 = async ({ request }) => {
  ...
  let audioItems = await db.audioItem.findMany({
    where: {
      id: {
  ...
  for (let a of audioItems) {
    let tagsWithoutTimestamps = a.tagsAsSubject.filter((t) => typeof t.subjectTimeMarkerSeconds != "number"), tagsWithTimestamps = a.tagsAsSubject.filter((t) => typeof t.subjectTimeMarkerSeconds == "number"), crowdsourcedInfo = `
${tagsWithoutTimestamps.length > 0 ? `Tags: 
...
function ExportToAtomCsv() {
  ...
  if (csvContent && typeof document < "u") {
  ...
  }, "Error: ", error), csvContent && "CSV downloading automatically...");
}
