...
function pscale {
    ...
    if [ -n "$NO_DOCKER" ]; then
        ...
        command="docker run -e PSCALE_SERVICE_TOKEN=$PSCALE_SERVICE_TOKEN -e PSCALE_SERVICE_TOKEN_NAME=$PSCALE_SERVICE_TOKEN_NAME  -e HOME=/tmp -v $HOME/.config/planetscale:/tmp/.config/planetscale -e PSCALE_ALLOW_NONINTERACTIVE_SHELL=true --user $(id -u):$(id -g) --rm -i $tty planetscale/pscale:latest $@"
    fi
