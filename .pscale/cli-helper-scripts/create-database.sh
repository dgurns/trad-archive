...
if [ -n "$PSCALE_SERVICE_TOKEN_NAME" ]; then
  echo "Granting access to new database for service tokens ..."
  pscale service-token add-access "$PSCALE_SERVICE_TOKEN_NAME" approve_deploy_request connect_branch create_branch create_comment create_deploy_request delete_branch read_branch read_deploy_request connect_production_branch  --database "$DB_NAME" --org "$ORG_NAME"
fi
