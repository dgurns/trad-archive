...
var import_react40 = require("@remix-run/react"), import_react41 = require("react"), CreateCommentForm = ({ parentAudioItem }) => {
  var _a;
  let formRef = (0, import_react41.useRef)(null), fetcher = (0, import_react40.useFetcher)();
...
var action3 = async ({ request }) => {
  ...
  if (!userId)
    return (0, import_node8.redirect)(`/login?redirectTo=${redirectTo}`);
  ...
  if (text ? parentAudioItemId || (error2 = "Parent audio item ID not defined") : error2 = "Comment cannot be empty", error2)
    return (0, import_node8.json)({ error: error2 }, { status: 400 });
  ...
  return (0, import_node8.json)({ comment }, { status: 201 });
};
...
var loader19 = async ({ request }) => {
  ...
  if (await db.user.findUnique({ where: { email } }))
    return session.flash("error", "There is already a user with this email address. Try logging in?"), (0, import_node10.redirect)("/signup", {
      headers: {
        "Set-Cookie": await commitSession(session)
      }
    });
  if (!new RegExp(/^([\w-]+(?:\.[\w-]+)*)@((?:[\w-]+\.)*\w[\w-]{0,66})\.([a-z]{2,6}(?:\.[a-z]{2})?)$/i).test(email))
    return session.flash("error", "Email is not valid"), (0, import_node10.redirect)("/signup", {
      headers: {
        "Set-Cookie": await commitSession(session)
      }
    });
  if (!(fullName.split(" ").length >= 2))
    return session.flash("error", "Please enter your full name"), (0, import_node10.redirect)("/signup", {
      headers: {
        "Set-Cookie": await commitSession(session)
      }
    });
  if (!(password === passwordConfirm))
    return session.flash("error", "Passwords don't match"), (0, import_node10.redirect)("/signup", {
      headers: {
        "Set-Cookie": await commitSession(session)
      }
    });
  if (!(password.length >= 8))
    return session.flash("error", "Password must be at least 8 characters long"), (0, import_node10.redirect)("/signup", {
      headers: {
        "Set-Cookie": await commitSession(session)
      }
    });
  ...
  return session.set("userId", newUser.id), (0, import_node10.redirect)(redirectTo || "/", {
    headers: {
      "Set-Cookie": await commitSession(session)
    }
  });
...
var import_react134 = require("@remix-run/react"), import_node11 = require("@remix-run/node"), import_bcryptjs4 = __toESM(require("bcryptjs"));
var loader21 = async ({ request }) => {
  ...
  return user ? user.passwordHashed ? import_bcryptjs4.default.compareSync(password, user.passwordHashed) ? (session.set("userId", user.id), (0, import_node11.redirect)(redirectTo || "/", {
    headers: {
      "Set-Cookie": await commitSession(session)
    }
  })) : (session.flash("error", "Incorrect password"), (0, import_node11.redirect)("/login", {
    headers: {
      "Set-Cookie": await commitSession(session)
    }
  })) : (session.flash("error", "This user does not have a password. TODO: Redirect to /reset-password page"), (0, import_node11.redirect)("/login", {
    headers: {
      "Set-Cookie": await commitSession(session)
    }
  })) : (session.flash("error", "Could not find a user with this email"), (0, import_node11.redirect)("/login", {
    headers: {
      "Set-Cookie": await commitSession(session)
    }
  }));
...
function Login() {
  let { error: error2 } = (0, import_react134.useLoaderData)(), transition = (0, import_react134.useTransition)(), { search } = (0, import_react134.useLocation)(), redirectTo = new URLSearchParams(search).get("redirectTo"), signUpLinkQueryParams = new URLSearchParams(redirectTo ? { redirectTo } : void 0);
  return /* @__PURE__ */ React.createElement(Layout_default, null, /* @__PURE__ */ React.createElement("h1", {
    className: "mb-6"
